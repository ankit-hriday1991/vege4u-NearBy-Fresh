...
import { getDirectionsUrl, calculateDistance } from './utils/location';
...
export default function App() {
  ...
  const { filteredShops, searchedVegetable, averageVegetablePrice } = useMemo(() => {
    ...
    let filtered = shops.filter(shop => {
      if (isNearbyEnabled && shop.freshnessRating < 4) {
        return false;
      }
    ...
    filtered = filtered.map(shop => ({
      ...shop,
      averagePrice: shop.commonVegetables.reduce((sum, veg) => sum + veg.price, 0) / shop.commonVegetables.length
    }));

    // Sort by distance if nearby enabled, otherwise by average price
  ...
  }, [shops, searchTerm, isNearbyEnabled, userLocation]);
